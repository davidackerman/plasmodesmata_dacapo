#!/usr/bin/env python3
"""
Flexible parallel version of the remeshing and distance matrix calculation.
This script can use different parallel backends: multiprocessing, joblib, or dask.
"""
...
try:
    ...
    logger.warning(
        "Optimized insertion functions not available, using original versions"
    )
...
def process_single_cell(
    ...
    Args:
        ...
        use_optimized: Whether to use optimized insertion functions when available
    ...
    try:
        ...
        if use_allow_duplicates:
            ...
            # Choose insertion function based on optimization settings
            if use_optimized and OPTIMIZED_AVAILABLE:
                insertion_func = get_fastest_insertion_function()
                logger.debug(f"Using optimized insertion function for cell {cell_id}")
            else:
                insertion_func = insert_points_into_mesh_original
                if use_optimized:
                    logger.debug(
                        f"Optimized functions not available, using original for cell {cell_id}"
                    )

            updated_vertices, updated_faces = insertion_func(
                cell_mesh, cell_plasmodesmata_coords
            )
        ...
        logger.info(
            f"Successfully processed cell {cell_id}: {len(cell_plasmodesmata_coords)} plasmodesmata in {total_time:.2f}s"
        )
        return results
...
def process_cells_multiprocessing(
    dataset: str,
    ...
    """
    Process multiple cells in parallel using multiprocessing.
    """
    ...
    logger.info(
        f"Processing {len(cell_ids)} cells using multiprocessing with {max_workers} workers"
    )
    ...
    process_func = partial(
        ...
        merged_df=merged_df,
        mesh_dir=mesh_dir,
        use_allow_duplicates=use_allow_duplicates,
    ...
    with Pool(processes=max_workers) as pool:
        results = list(
            tqdm(
                pool.imap(process_func, cell_ids),
                total=len(cell_ids),
...
def process_cells_joblib(
    ...
    """
    Process multiple cells in parallel using joblib.
    """
    try:
        from joblib import Parallel, delayed
    except ImportError:
        raise ImportError(
            "joblib is required for this backend. Install with: pip install joblib"
        )
    ...
    logger.info(
        f"Processing {len(cell_ids)} cells using joblib with {max_workers} workers"
    )

    # Execute in parallel
    results = Parallel(n_jobs=max_workers, verbose=1)(
...
def process_cells_dask(
    ...
    try:
        ...
        raise ImportError(
            "dask is required for this backend. Install with: pip install 'dask[complete]'"
        )
    ...
    logger.info(
        f"Processing {len(cell_ids)} cells using Dask with {max_workers} workers"
    )

    # Create delayed tasks
    delayed_process = delayed(process_single_cell)
    tasks = [
        delayed_process(
            cell_id, merged_df, mesh_dir, use_allow_duplicates, save_results, output_dir
        )
...
def process_cells_sequential(
    ...
    """
    Process multiple cells sequentially (no parallelization).
    """
    # Load and prepare data
    merged_df, cell_file, mesh_dir = load_and_prepare_data(dataset)
    ...
    logger.info(f"Processing {len(cell_ids)} cells sequentially")
...
def process_cells_parallel(
    ...
    """
    Process multiple cells in parallel using the specified backend.

    Args:
        dataset: Dataset name
        cell_ids: List of cell IDs to process. If None, process all cells with data.
        max_workers: Maximum number of parallel workers
        backend: Parallel backend ("multiprocessing", "joblib", "dask", "sequential")
        use_allow_duplicates: Whether to use the allow_duplicates insertion method
        use_optimized: Whether to use optimized insertion functions when available
        save_results: Whether to save intermediate results
        output_dir: Directory to save results
    ...
    if backend == "multiprocessing":
        results = process_cells_multiprocessing(
            dataset,
            cell_ids,
            max_workers,
            use_allow_duplicates,
            save_results,
            output_dir,
        )
    elif backend == "joblib":
        results = process_cells_joblib(
            dataset,
            cell_ids,
            max_workers,
            use_allow_duplicates,
        ...
        results = process_cells_sequential(
            dataset, cell_ids, use_allow_duplicates, save_results, output_dir
        )
    else:
        raise ValueError(
            f"Unknown backend: {backend}. Choose from: multiprocessing, joblib, dask, sequential"
        )
    ...
    logger.info(
        f"Completed processing {len(results)} cells in {total_time:.2f}s using {backend}"
    )
        ...
        summary_data.append(
            {
                ...
                "insert_time": timing_info.get("insert_time", 0),
                "distance_time": timing_info.get("distance_time", 0),
            }
        )
    ...
    for backend in backends:
        ...
        try:
            ...
            results = process_cells_parallel(
                ...
                max_workers=2 if backend != "sequential" else 1,
                backend=backend,
                use_allow_duplicates=True,
                save_results=True,
                output_dir=f"remesh_results_{backend}",
            )

            # Analyze results
            summary_df = analyze_results(
                results, output_dir=f"remesh_results_{backend}"
            )
            print(f"\nSummary Statistics for {backend}:")
            print(summary_df)
