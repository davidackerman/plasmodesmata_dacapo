...
for dataset in datasets:
    ...
    # Generate scatter plots for each quantity vs. COM coordinates for this dataset.
...
for ds, regions in datasets.items():
    # Construct file path and load the CSV file for each dataset
    pd_file = f"/nrs/cellmap/ackermand/cellmap/analysisResults/leaf-gall/{ds}/plasmodesmata_cleaned_lines_assigned_to_2_nearest_cells.csv"
    df = pd.read_csv(pd_file)
    ...
    for region, limits in regions.items():
        ...
        region_coords = coords[(coords[:, 2] >= limits[0]) & (coords[:, 2] < limits[1])]
        ...
        dists, g_r, total_valid_pairs = calculate_rdf(region_coords, max_distance, bins)
        print(
...
scatter = ax.scatter(closest[:,0], closest[:,1], closest[:,2], c=dists[num_vertices:], cmap='viridis')
